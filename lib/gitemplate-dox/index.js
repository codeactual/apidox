...
GitemplateDox.prototype.buildSees = function(md, comment) {
  ...
  sees.forEach(function(description) {
    md.push('- ' + self.linkSymbols(description));
  });
};
 ...
 * @api private
 */
GitemplateDox.prototype.linkSymbols = function(text) {
  var self = this;
  ...
  methods.forEach(function(method) {
    var regex = new RegExp('(^|\\s)(' + method.replace(/\./, '\\.') + ')(\\s|$)', 'g');
    text = text.replace(regex, sprintf('$1[%s](#%s)$3', method, self.slug(method)));
  });
  return text;
};
