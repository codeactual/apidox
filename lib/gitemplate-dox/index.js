...
GitemplateDox.prototype.convertParams = function(comment) {
  ...
  names.forEach(function(name, idx) {
    ...
    self.lines.push(sprintf(
        ...
        param.types.join(' | '),
        name,
        param.description ? ' ' + self.linkMethodNames(param.description, comment) : ''
    ));

    if (param.overflow.length) {
      self.newline();
      param.overflow.forEach(function(line) {
        self.lines.push(self.linkMethodNames(line, comment));
      });
...
GitemplateDox.prototype.convertReturns = function(comment) {
  ...
  this.p(sprintf(
    '`{%s}`%s',
    returns.types.join(' | '),
    returns.description ? ' ' + this.linkMethodNames(returns.description, comment) : ''
  ));
...
GitemplateDox.prototype.convertSees = function(comment) {
  ...
  sees.forEach(function(description) {
    self.lines.push('- ' + self.linkMethodNames(description, comment));
  });
};
