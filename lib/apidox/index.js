  ...
  comment.tags.forEach(function(tag) {
    if ('param' === tag.type) {
      var lines = tag.string.split('\n'),
          parts = dox.extractTagParts(lines[0]);
      parts.shift();
      parts.shift();
      ...
      for (var i = 1; i < lines.length; i += 1) {
        ...
        var firstStrChar = trimmed.slice(0, 1);
        if ('-' === firstStrChar || '*' === firstStrChar) {
          trimmed = '  ' + lines[i];
        }
        self.params[method][tag.name].overflow.push(trimmed);
      }
    }
  });
};
  ...
  comment.tags.forEach(function(tag) {
    if ('return' === tag.type) {
      ...
      for (var i = 1; i < lines.length; i += 1) {
        ...
        self.returns[method].overflow.push(trimmed);
      }
    }
  });
};
  ...
  comment.tags.forEach(function(tag) {
    if ('throws' === tag.type) {
      ...
      var description = parts.join(' '),
          throwId = tag.types.join('|') + description;
      ...
      for (var i = 1; i < lines.length; i += 1) {
        ...
        self.throws[method][throwId].overflow.push(trimmed);
      }
    }
  });
};
